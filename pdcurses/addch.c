...
int _waddch(WINDOW *win, chtype text, chtype attr, int x, int y) {
  ...
  /* If the character is not a space, update the first and last
...
int waddch(WINDOW *win, const chtype ch) {
  ...
  // Check if window pointer is valid and curses library is initialized
  if (!win || !SP)
    return ERR;

  // Extract character and attributes from ch
  chtype text = ch & A_CHARTEXT;
  if (text == 0x200b)
    return OK;

  // Check if current position is within window boundaries
  int cw = mk_wcwidth(text);
  int x = win->_curx, y = win->_cury;
  if (y > win->_maxy || y < 0 || x > win->_maxx)
    return ERR;

  // Extract attributes from ch
  chtype attr = ch & A_ATTRIBUTES;
  ...
  if (cw == 2) {
    ...
    if (x == -1) {
      ...
      _waddch(win, text, attr, x, y);
      ++x;
      text = 0x200b;
    }
  } else if (x < 0)
    return ERR;
  else if (!SP->raw_out && !(ch & A_ALTCHARSET) &&
           (text < ' ' || text == 0x7f)) {
  ...
  // Add character to window at current position
  _waddch(win, text, attr, x, y);

  // Update position and handle scrolling
  if (++x >= win->_maxx) {
    x = 0;
...
cleanup:
  ...
  // Refresh and synchronize window if necessary
  if (win->_immed)
    wrefresh(win);
  if (win->_sync)
    wsyncup(win);
